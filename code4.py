# 1. 이 코드를 기반으로 N장의 원본 이미지를 Blip으로 텍스트를 추출해 맥락 파악 
# 2. 부분 적으로 키워드를 가져와 시적인 (셰익스피어의 문장처럼 아름다운) + 조금은 낯선 (존 라프만 처럼 현대미술적인) 하나의 텍스트로 재배치해 이미지를 생성ㅎ나다.
# (Ai가 원본 이미지를 기반으로 생성한 시적인 프롬프트 (셰익스피어처럼 아름다운)+ 고정된 프롬프트 (존라프만 스타일처럼 현대미술적인)
# + 색 등의 조정값에 대한 보정도 AI의 판단 + 고정 판단을 같이 가는 전반적으로 AI+트레시 이미지 팀의 크로스 체크 방식으로 전개
